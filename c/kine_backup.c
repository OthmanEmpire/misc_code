...
// THIS FUNCTION IS COMPLETE AND WORKING AS INTENDED
...
int terminal()		// This is the menu of KINE, not asthetically beautiful I must say myself :o
{
	...
	printf("\nYou are now in the KINE terminal.\n");		// Perhaps make a somewhat visual UI of navigation?
	printf("\nThe choices available to you are:\n\n1. Inclined planes\n2. Vertical trajectory\n3. Parabolic trajectory");
	printf("\n4. Horizontal planes\n5. Pulleys\n6. Manual injection of SUVAT variables\n");
	...
	while( (option > 6) || (option < 1) )		// Filter to catch out noob input
	{
		printf("\n\t>>>  Please enter a NON-noob destination.  <<<\t\n\nNumber: ");
	...
	switch(option)
	{
		case '1' :						// "option" is of type integer. While '1' is of type character. Therefore the mapping between them will be incorrect
			// Inclined_Plane();
			break;
...
// THE FUNCTION BELOW IS ALMOST COMPLETE //
...
{												// because "Realloc" (below) has the potential of changing the address that the pointer in int main is pointing to. If we 
	...
	while( fread(temp_data, sizeof(char), 1, fp) == 1 ) 
	{
		...
		{	
			...
			{
				data_start = temp;
				temp_data = temp + init_size*(n - 1);			// The new chunk of free data is between address init_size*(n - 1) & initsize*n. Therefore, we can start writing
	...
	*data = data_start;			// Assigning the address of the variable from int main to the local variable
